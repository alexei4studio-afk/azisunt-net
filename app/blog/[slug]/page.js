...
export default async function PostPage({ params }) {
  // CRITIC: În Next.js App Router, trebuie să facem await la params
  const { slug } = await params;
  
  // Preluăm datele reale ale articolului
  const post = await getPostData(slug);

  if (!post) {
    return <div className="text-white pt-40 text-center font-display italic">Articolul nu a fost găsit.</div>;
  }
