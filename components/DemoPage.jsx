 ...
 *   gallery: [{ src, alt }],           // optional — placeholders shown if empty
 *   reviews: [{ name, text, stars }],
 *   cta: "Programează-te acum",        // optional override
 * }
 */
          ...
          {gallery.length > 0
            ...
            : [0, 1, 2, 3].map((i) => (
                <div
                  ...
                  style={{ background: `linear-gradient(135deg, ${primaryColor}18, ${accentColor}10)`, border: `1px solid ${primaryColor}25` }}
                >
                  Foto {i + 1}
                </div>
              ))}
      ...
      <section className="px-4 pb-28 max-w-lg mx-auto">
        ...
        <div className="bg-white/[0.04] border border-white/[0.07] rounded-2xl p-5 flex flex-col gap-3">
          <a href={telLink} className="flex items-center gap-3 text-sm font-medium">
            <span className="w-8 h-8 rounded-xl flex items-center justify-center text-base flex-shrink-0" style={{ ...accentBg, opacity: 0.9 }}>📞</span>
            {phone}
          </a>
          {address && (
            <div className="flex items-center gap-3 text-sm font-medium text-white/60">
              <span className="w-8 h-8 rounded-xl bg-white/[0.06] flex items-center justify-center text-base flex-shrink-0">📍</span>
              {address}
            </div>
          )}
        </div>
