...
export function getSortedPostsData() {
  ...
  const posts = mdFiles.map((fileName) => {
    ...
    console.log("[posts] Citire fisier:", filePath);   // ← log cerut
...
export async function getPostData(slug) {
  ...
  console.log("[posts] Citire fisier:", fullPath);     // ← log cerut

  if (!fs.existsSync(fullPath)) {
    console.error("[posts] ❌ Fișier inexistent:", fullPath);
    // Afișăm ce există în folder ca să depanăm mai ușor
    try {
  ...
  if (!contentHtml.trim()) {
    console.warn("[posts] ⚠️  contentHtml este GOL pentru:", safeSlug);
  }
