...
function readFileLF(filePath) {
  // .trim() elimină orice BOM, spații sau newline-uri înainte de "---"
...
export function getSortedPostsData() {
  ...
  const posts = mdFiles.map((fileName) => {
    ...
    const raw = readFileLF(filePath); // trim() aplicat aici
...
export async function getPostData(slug) {
  ...
  const raw = readFileLF(fullPath); // trim() aplicat și aici
